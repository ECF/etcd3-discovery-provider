...
public class Etcd3Node {
	...
	private final Etcd3Node[] nodes;

	private Etcd3Node[] createNodes(JSONObject jsonObject) throws JSONException {
		JSONArray array = jsonObject.optJSONArray(NODES_KEY);
		if (array != null) {
			List<Etcd3Node> nodes = new ArrayList<Etcd3Node>();
			for (int i = 0; i < array.length(); i++)
				nodes.add(new Etcd3Node(array.getJSONObject(i)));
			return nodes.toArray(new Etcd3Node[nodes.size()]);
		} else
			return null;
	}
	...
	public Etcd3Node(JSONObject jsonObject) throws JSONException {
	...
	public Etcd3Node[] getNodes() {
		return nodes;
	}

	@Override
	public String toString() {
		return "Etcd3Node[createdIndex=" + createdIndex + ", directory=" //$NON-NLS-1$ //$NON-NLS-2$
