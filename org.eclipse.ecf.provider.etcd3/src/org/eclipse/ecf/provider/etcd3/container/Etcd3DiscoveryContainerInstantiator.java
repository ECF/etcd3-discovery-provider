...
public class Etcd3DiscoveryContainerInstantiator implements
		IContainerInstantiator {

	public static final String NAME = "ecf.discovery.etcd"; //$NON-NLS-1$

	private Etcd3DiscoveryContainer createContainer(Etcd3DiscoveryContainerConfig config) throws ContainerCreateException {
		try {
			if (config == null) return new Etcd3DiscoveryContainer();
			else return new Etcd3DiscoveryContainer(config);
		} catch (Exception e) {
	...
	public IContainer createInstance(ContainerTypeDescription description,
		...
		Etcd3DiscoveryContainer result = null;
		if (parameters == null || parameters.length == 0) {
			result = createContainer(null);
		} else if (parameters[0] instanceof Etcd3DiscoveryContainerConfig) {
			Etcd3DiscoveryContainerConfig edcc = (Etcd3DiscoveryContainerConfig) parameters[0];
			if (edcc != null) 
